...
const OrderScreen = () => {
  ...
  return (
    <div className={classes.container}>
      ...
      <tr className={classes.fullrow}>Sales Tax: ${(cartTotal * .07).toFixed(2)}</tr>
      
      </table>
      <form >
      <h2>Payment Information</h2>  
      <select id="payment-type" name="payment-type" onChange={changeSelectPaymentHandler} ref={paymentRef}>
      <option value="Cash">Cash</option>
      <option value="Credit Card">Credit Card</option>
      <option value="Check">Check</option>      
      </select>
        <div className={classes.cardinfo}>
        {selected === "Credit Card" && <label for="card-number">Card Number</label>}
        {selected === "Credit Card" && <input type="text" id="card-number" name="card-number" ref={cardNumberRef} />}
        {selected === "Credit Card" && <label for="exp-date">Exp Date</label>}
        {selected === "Credit Card" && <input type="text" id="exp-date" name="exp-date" ref={expDateRef} />}
        {selected === "Credit Card" && <label for="">Security Code</label>}
        {selected === "Credit Card" && <input type="text" name="security-code" id="security-code" />}
        </div>
      </form>
